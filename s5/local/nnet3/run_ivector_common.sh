...
for f in data/${train_set}/feats.scp ${gmm_dir}/final.mdl; do
  ...
  cp -R data/${train_set} data/${train_set}_sp
  echo "$0: making MFCC features for low-resolution speed-perturbed data"

  steps/make_mfcc_pitch.sh --cmd "$train_cmd" --nj 18 data/${train_set}_sp \
...
if [ $stage -le 3 ]; then
  ...
  for datadir in ${train_set}_sp ${test_sets}; do
    steps/make_mfcc_pitch.sh --nj 18 --mfcc-config conf/mfcc_hires.conf \
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 18 \
    --num-frames 700000 \
    --num-threads 8 \
...
echo 'start train_ivector_extractor'
echo `date`
...
if [ $stage -le 5 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "run.pl" --nj 18 \
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 18 \
